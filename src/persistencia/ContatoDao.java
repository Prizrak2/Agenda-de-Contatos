...
public class ContatoDao implements Icrud{
    ...
    public ArrayList<Contato> consultar(Contato filtro) throws Exception {
        BufferedWriter buffArquivo = null;
        ...
        try {
            ...
            // Criar e escrever no arquivo auxiliar .csv
            Path caminhoDoArquivo = encontrarCaminhoDoArquivo("contatos_filtrados.csv");
            FileWriter arquivo = new FileWriter(caminhoDoArquivo.toFile());
            buffArquivo = new BufferedWriter(arquivo);

            // Escrever os contatos filtrados no arquivo CSV
            for (Contato contato : contatosFiltrados) {
                String linhaContato = contato.getNome() + ";" +
                                      contato.getTelefone() + ";" +
                                      contato.getEmail() + ";" +
                                      contato.getEndereco().toStringArquivo();
                buffArquivo.append(linhaContato).append("\n");
            }
            ...
            throw new Exception("Erro na ContatoDAO - Consultar - Erro na manipulação do arquivo", erro);
        }
